...
function ContactSection({ theme }) {
  ...
  const lnk = { display:"flex", alignItems:"center", gap:10, color:"rgba(245,232,215,.75)", textDecoration:"none", fontSize:15, padding:"10px 0", borderBottom:`1px solid ${border}`, transition:"color .2s" };

  return (
    <div id="contact" style={{ padding:"80px 24px", borderTop:`1px solid ${border}`, background: isEpoxi ? "rgba(255,255,255,.01)" : "rgba(0,0,0,.05)" }}>
      <div style={{ maxWidth:900, margin:"0 auto" }}>
        <h2 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:44, fontWeight:400, textAlign:"center", marginBottom:16, color: isEpoxi ? "#9EEAF9" : "#D4A855" }}>Contacto</h2>
        <Divider color={isEpoxi ? "#9EEAF9" : "#D4A855"} />
        <div style={{ display:"flex", gap:24, flexWrap:"wrap" }}>
          {/* Madeira */}
          <div style={{ flex:"1 1 280px", background:cardBg, border:"1px solid rgba(212,168,85,.25)", borderRadius:16, padding:28, backdropFilter:"blur(8px)" }}>
            <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:24, color:"#D4A855", marginBottom:20 }}>🪵 Madeira</h3>
            {[
              { href:"https://www.facebook.com/IdeiasMaravilha", icon:"📘", text:"Facebook — Ideias Maravilha", hoverC:"#D4A855" },
              { href:"https://instagram.com/marcomaravilh?igsh=NWUza.J2NHF0ando", icon:"📸", text:"Instagram — @marcomaravilh", hoverC:"#D4A855" },
              { href:"tel:+351916943697", icon:"📞", text:"+351 91 69 43 697", hoverC:"#D4A855", last:true },
            ].map(({ href, icon, text, hoverC, last }) => (
              <a key={href} href={href} target={href.startsWith("tel") ? undefined : "_blank"} rel="noopener"
                ...
                onMouseLeave={e => e.currentTarget.style.color = "rgba(245,232,215,.75)"}
              ><span style={{ fontSize:20 }}>{icon}</span>{text}</a>
            ))}
          </div>
          {/* Epóxi */}
          <div style={{ flex:"1 1 280px", background:cardBg, border:"1px solid rgba(0,184,255,.2)", borderRadius:16, padding:28, backdropFilter:"blur(8px)" }}>
            <h3 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:24, background:"linear-gradient(135deg,#9EEAF9,#7B2FBE)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", marginBottom:20 }}>💎 Epóxi</h3>
            {[
              { href:"https://www.facebook.com/share/1ABJTywnqe/", icon:"📘", text:"Facebook — Epoxi_by_Gi", hoverC:"#9EEAF9" },
              { href:"https://www.instagram.com/epoxiebarbantebygi?igsh=MTdpdjh6anRvcngxbg==", icon:"📸", text:"Instagram — @epoxiebarbantebygi", hoverC:"#9EEAF9" },
              { href:"tel:+351916943697", icon:"📞", text:"+351 91 69 43 697", hoverC:"#9EEAF9", last:true },
            ].map(({ href, icon, text, hoverC, last }) => (
              <a key={href} href={href} target={href.startsWith("tel") ? undefined : "_blank"} rel="noopener"
                style={{ ...lnk, ...(last ? { borderBottom:"none" } : {}) }}
                onMouseEnter={e => e.currentTarget.style.color = hoverC}
